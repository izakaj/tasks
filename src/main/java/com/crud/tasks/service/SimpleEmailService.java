...
public class SimpleEmailService {
    ...
    private MimeMessagePreparator createDailyInfoMimeMessage(final Mail mail, long numberOfTasks) {
        return mimeMessage -> {
            MimeMessageHelper messageHelper = new MimeMessageHelper(mimeMessage);
            messageHelper.setTo(mail.getMailTo());
            messageHelper.setSubject(mail.getSubject());
            messageHelper.setText(mailCreatorService.buildDailyTasksNumberInfoEmail(mail.getMessage(), numberOfTasks), true);
        };
    }
    ...
    public void sendInformationEmail (final Mail mail, long numberOfTasks){
        LOGGER.info("Starting email preparation...");
        try {
            MimeMessagePreparator mimeMessage = createDailyInfoMimeMessage(mail, numberOfTasks);
            LOGGER.info("Mime Message created.");
            javaMailSender.send(mimeMessage);
            LOGGER.info("Email has been sent.");
        } catch (MailException e) {
